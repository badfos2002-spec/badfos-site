...
const babyMockups: Record<string, string> = {
  ...
  lightblue: '/assets/baby-blue.webp',
...
export default function BabyDesignerPage() {
  ...
  const setQty = (sizeId: string, value: number) => {
    ...
    setSizeQuantities(prev => {
      const updated = { ...prev }
      if (v === 0) delete updated[sizeId]
      else updated[sizeId] = Math.min(99, v)
      return updated
    })
  }
  ...
  const mockupSrc = babyMockups[selectedColor] || '/assets/baby-white.webp'
  ...
  const stepContent = (() => {
    switch (currentStep) {
        ...
        return (
          <div>
            <p className="text-sm text-gray-500 mb-4">
              {selectedColor ? `נבחר: ${BABY_COLORS.find(c => c.id === selectedColor)?.name}` : 'בחרו את צבע בגד הגוף'}
            </p>
              ...
              {BABY_COLORS.map(color => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {isSelected && <Check className={`w-5 h-5 ${color.id === 'white' ? 'text-gray-800' : 'text-gray-700'}`} strokeWidth={3} />}
                    </div>
      ...
      case 3:
        return (
          <div>
            <p className="text-sm text-gray-500 mb-4">בחרו מידה וכמות לכל מידה (אפשר לערבב מידות).</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {BABY_SIZES.map(size => {
                const qty = sizeQuantities[size.id] || 0
                return (
                  <div
                    key={size.id}
                    className={`p-3 border-2 rounded-lg flex items-center justify-between transition-all ${
                      qty > 0 ? 'border-[#fbbf24] bg-yellow-50' : 'border-gray-200 bg-white'
                    }`}
                  >
                    <span className="font-semibold text-sm text-[#1e293b]">{size.name}</span>
                    <div className="flex items-center gap-2">
                      <Button
                        variant="outline"
                        size="icon"
                      ...
                      <input
                        ...
                        value={qty || ''}
                        placeholder="0"
                        onChange={(e) => setQty(size.id, parseInt(e.target.value) || 0)}
                        className="w-12 text-center font-bold h-8 rounded-md border border-input bg-transparent px-2 text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                      />
                      <Button
                        variant="outline"
                        size="icon"
            ...
            {totalQuantity === 0 && <p className="text-sm text-red-500 mt-4">יש לבחור מידה אחת לפחות.</p>}
          </div>
        )
