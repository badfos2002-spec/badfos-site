...
const stepConfig = [
  { title: 'בחר סוג תיק', icon: ShoppingBag },
...
const LIGHT_COLOR_IDS = ['white', 'beige']

export default function ToteDesignerPage() {
  ...
  const availableColors = useMemo(() => {
    ...
    const allowed = TOTE_COLOR_FILTER[selectedType] || []
    return TOTE_COLORS.filter(c => allowed.includes(c.id))
  }, [selectedType])

  const availableAreas = useMemo(() => {
    if (!selectedType) return [TOTE_DESIGN_AREAS[0]]
    const allowed = TOTE_AREA_FILTER[selectedType] || ['front_full']
    return TOTE_DESIGN_AREAS.filter(a => allowed.includes(a.id))
  }, [selectedType])
  ...
  const handleTypeChange = (typeId: string) => {
    ...
    if (selectedColor) {
      const allowed = TOTE_COLOR_FILTER[typeId] || []
      if (!allowed.includes(selectedColor)) setSelectedColor('')
    }
    const allowedAreas = TOTE_AREA_FILTER[typeId] || ['front_full']
    ...
    switch (currentStep) {
        ...
        return (
          <div>
            <p className="text-sm text-gray-500 mb-4">
              {selectedType ? `נבחר: ${TOTE_TYPES.find(t => t.id === selectedType)?.name}` : 'בחרו סוג תיק'}
            </p>
            ...
            {!selectedType && <p className="text-sm text-red-500 mt-4">יש לבחור סוג תיק כדי להמשיך.</p>}
          </div>
        )
      ...
      case 2:
        return (
          <div>
            <p className="text-sm text-gray-500 mb-4">
              {selectedColor ? `נבחר: ${availableColors.find(c => c.id === selectedColor)?.name}` : 'בחרו את צבע התיק'}
            </p>
      ...
      case 4:
        return (
          <div>
            <p className="text-sm text-gray-500 mb-4">בחרו כמות תיקים.</p>
            ...
            <div className="bg-yellow-50 border-2 border-[#fbbf24]/30 rounded-xl p-6">
              ...
              <div className="flex items-center justify-center gap-3">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => setQuantity(q => Math.max(TOTE_MIN_QUANTITY, q - 1))}
                  disabled={quantity <= TOTE_MIN_QUANTITY}
                ...
                <input
                  type="number"
                  min={TOTE_MIN_QUANTITY}
                  value={quantity}
                  onChange={e => {
                    const v = parseInt(e.target.value, 10)
                    if (Number.isFinite(v)) setQuantity(Math.max(TOTE_MIN_QUANTITY, v))
                    else setQuantity(TOTE_MIN_QUANTITY)
                  }}
              ...
              <div className="text-center mt-4">
                ...
                <div className="text-xs text-gray-500 mt-1">סה&quot;כ {total}₪ עבור {quantity} תיקים</div>
              </div>
            </div>
          </div>
        )
  ...
  // No dedicated 2D bag photo — if WebGL fails, show the uploaded design on a
  // neutral canvas-coloured card so the user still sees their artwork.
  const MockupImage = () => (
    <div
